...
{
    // There are multiple ways to register a POM object, this is one way that I found easier to understand than the other ways, for other examples try looking at RegionKit's POM objects
    ...
    public class TubusObjectType : ManagedObjectType
    {
        ...
        // Because we are already adding the object to the room, we return null after our logic to tell POM that we don't need an object added, this is also why we don't need the objectType in our constructor
        //  For a simple UAD you don't need to override this, as passing in the type of UAD as the objectType will allow POM to add it to the room on its own (..I think)
    ...
    public class TubusData : ManagedData
    {
        ...
        private static readonly List<ManagedField> tubusFields = [
            new IntegerField("seed", -1, 10000, 1, ManagedFieldWithPanel.ControlType.slider, "Seed")];
    ...
    // The ManagedRepresentation class for the Tubus Tree, this controls the UI interface by creating the panel and controls the for the ManagedFields
